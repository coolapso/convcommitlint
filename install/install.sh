...
FILE="convcommitlint_${VERSION:1}_${OS}_${ARCH}.tar.gz"
...
## Install the binary
if ! chmod +x convcommitlint; then
  echo "Failed to make convcommitlint executable"
  exit 1
fi

if ! mv convcommitlint "$INSTALL_DIR" ; then
  echo "Failed to move convcommitlint to $INSTALL_DIR"
  exit 1
fi
